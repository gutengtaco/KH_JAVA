...
public class PhoneRun {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 휴대폰명이 null 이 아닐 경우에만 목록이 조회되도록 조치 => NullPointerException 발생 
			// arr[i] 에 들어있는것이 null 일 경우에는 break; 를 걸겠다. => 로직 변경!
			if(arr[i] != null) {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// => 애초에 비교를 하기 전에 arr[i] 자체가 애초에 null 인지 아닌지 먼저 판별을 하고 비교하면됨
			if(arr[i] == null) { // 비교를 애초에 막자
				continue;
			}
			else {
				// 있는 기기명을 입력한 경우 그리고 시리즈도 있는경우
		...
		if(count == 0) {
			...
			if(answer == 'y') {
				...
				// 배열로부터 현재 그 휴대폰의 정보를 지워주면됨 => 해당 그 배열 방의 값에 null 을 넣어 연결고리를 끊자!
				arr[pick] = null;
			}
			else {
